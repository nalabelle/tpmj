...
public class DoubleStat implements java.io.Serializable
{
    ...
    public DoubleStat(DoubleStat dStat) {
        this.n = dStat.n;
        this.total = dStat.total;
        this.mean = dStat.mean;
        this.totalSq = dStat.totalSq;
        this.totalVar = dStat.totalVar;
        this.min = dStat.min;
        this.max = dStat.max;
    }
    ...
    /**
     * This test demonstrates how it is possible to combine DoubleStats.
     * Note the following:
     * <ul>
     * <li> The results for the single run are the same for the
     *      DoubleStat of DoubleStats. 
     *      </li>
     * 
     * <li> The meanStDev of each sub-DoubleStat is roughly the same
     *      as the SampleStDev of the DoubleStat of Means.  This shows
     *      that the meanStDev successfully estimates the stdev of the means.
     *      </li>
     * 
     * <li> The meanStDev of the DoubleStat of Means is roughly the 
     *      same as the meanStDev of the DoubleStat of DoubleStats, which
    ...
    public static void main( String[] args )
    {
        ...
        if ( args.length > 2 )
        {
            seedInit = Long.parseLong( args[2] );
        }
        ...
        DoubleStat aStat = new DoubleStat();
        ...
        System.out.println( "DoubleStat: " + aStat );

        DoubleStat[] a10Stat = new DoubleStat[P];
        for ( int i = 0; i < P; i++ )
        {
            a10Stat[i] = new DoubleStat();
        }
        ...
        DoubleStat aSuperStat = new DoubleStat();
        DoubleStat aMeanStat = new DoubleStat();

        for ( int i = 0; i < P; i++ )
        {
            System.out.println( "adding " + a10Stat[i] );
            aSuperStat.addDStat( a10Stat[i] );
            aMeanStat.addSample( a10Stat[i].getMean() );
        }

        System.out.println( "DoubleStat of DoubleStats: " + aSuperStat );

        System.out.println( "DoubleStat of Means: " + aMeanStat );
    }
}
